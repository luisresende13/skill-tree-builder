...
export const useSkillTree = (domain: string) => {
  ...
  const handleConfirmSelection = useCallback(
    async (selectedSkillNames: string[]) => {
      ...
      const categorySkills: Skill[] = currentCategory.skills.map(
        (apiSkill: ApiSkill) => ({
          ...apiSkill,
          status: newKnownFromSelection.has(apiSkill.name)
            ? 'known'
            : 'learning',
        }),
      );

      const newlyKnownSkills = categorySkills
        .filter((s) => s.status === 'known')
        .map((s) => s.name);
      const updatedKnownSkills = [
        ...new Set([...knownSkills, ...newlyKnownSkills]),
      ];
      ...
      setLearningSkills((prev) => [
        ...prev,
        ...categorySkills.filter((s) => s.status === 'learning'),
      ]);
      setCurrentCategory(null);

      try {
        const allExistingSkillNames = [
          ...tree.map((s) => s.name),
          ...currentCategory.skills.map((s) => s.name),
        ];
        const nextCategory = await getNextSkills(
          domain,
          updatedKnownSkills,
          allExistingSkillNames,
        );
