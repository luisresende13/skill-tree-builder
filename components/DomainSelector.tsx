...
const DomainSelector: React.FC<DomainSelectorProps> = ({ onStart }) => {
  ...
  const filteredDomains = useMemo(() => {
    ...
    for (const category in DOMAIN_CATEGORIES) {
      const filtered = DOMAIN_CATEGORIES[category as keyof typeof DOMAIN_CATEGORIES].filter(domain =>
        domain.toLowerCase().includes(lowercasedFilter)
      );
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 p-4 sm:p-6 md:p-8">
      <div className="w-full max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <MapIcon />
          <h1 className="text-4xl font-extrabold text-white mt-4">Choose Your Domain(s)</h1>
          <p className="text-slate-400 mt-2 max-w-2xl mx-auto">
            Select one or more areas of interest to start building your skill map. You can also add your own custom domains.
          </p>
        </div>
        ...
        <div className="sticky top-0 z-10 bg-slate-900/80 backdrop-blur-sm py-4 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <form onSubmit={handleAddCustomDomain} className="flex gap-2">
                    ...
                    />
                    <button type="submit" className="bg-emerald-600 text-white font-semibold px-4 rounded-lg hover:bg-emerald-500 transition-colors disabled:bg-slate-700 disabled:cursor-not-allowed" disabled={!customDomainInput.trim()}>
                        Add
                    </button>
                </form>
            </div>
            ...
            <div className="mt-4 p-3 bg-slate-950/50 rounded-lg">
                <div className="flex flex-wrap gap-2 items-center">
                    <span className="text-sm font-medium text-slate-400 mr-2">Selected:</span>
                    {selectedArray.map(domain => (
                    <span key={domain} className="flex items-center gap-2 bg-emerald-500/10 text-emerald-400 text-sm font-medium px-3 py-1 rounded-full">
                        {domain}
                        <button onClick={() => handleToggleDomain(domain)} className="text-emerald-400/70 hover:text-emerald-400">
                           <XIcon />
                        </button>
                    </span>
                    ))}
        ...
        <div className="space-y-6">
          {Object.entries(filteredDomains).map(([category, domains]) => (
            <div key={category}>
              <h3 className="text-xl font-bold text-slate-300 border-b-2 border-slate-800 pb-2 mb-4">{category}</h3>
        ...
        {Object.keys(filteredDomains).length === 0 && searchTerm && (
            <div className="text-center py-10">
                <p className="text-slate-400">No domains found for "{searchTerm}".</p>
                <p className="text-sm text-slate-500 mt-2">You can add it as a custom domain above.</p>
            </div>
        )}
